...
title_clean_sci <- sal_clean_filt |> 
  ...
  mutate(
    ...
    title_general = str_extract(
      title_,
      '((Associate|Senior|Principal|Staff)?\\s?(Data\\s?)?)*Scientist\\s?[I]*'
    ) |> 
      str_trim(),
...
title_clean <- sal_clean_filt |> 
  left_join(
    title_clean_sci |> 
      select(timestamp, title_category, title_general, title_detail), 
    by = 'timestamp')
...
Look at the remaining roles that have not been processed:
...
address non cleaned responses by just using the original response

```{r}
sal_clean_filt <- title_clean_sdv |> 
  mutate(
    title_category = ifelse(is.na(title_category), 'Other', title_category),
    title_general = ifelse(is.na(title_general), role_title_of_current_position, title_general),
    title_detail = ifelse(is.na(title_detail), role_title_of_current_position, title_detail)
  ) 
```
...
The survey tried to get "hubs", but most of the data is missing if relying on this information.
...
if (!file.exists(file)) {
  ...
  get_location <- function(.df, location) {
    ...
    .df |> 
      mutate(.results = furrr::future_map(
        ...
        .options = furrr_options(scheduling = 6L, seed = TRUE))
      ) |> 
  ...
  ## what_country_do_you_work_in
  tic();query2 <- sal_clean_filt |> 
    filter(!is.na(what_country_do_you_work_in)) |> 
    select(timestamp, what_country_do_you_work_in) |> 
    get_location(what_country_do_you_work_in);toc()
  
  file <- here::here('data', 'what_country_do_you_work_in.csv')
  query2 |>  write_csv(file)
} else {
  query <- read_csv(file)
}
query  |>  distinct() 
...
```{r}
sal_clean_filt <- sal_clean_filt |>  
  left_join(distinct(query) |>  filter(!is.na(country)), by = c('where_are_you_located'))
```
...
sal_clean_filt |>  
  count(where_are_you_located) |>  
  arrange(desc(n)) |> 
  pull(where_are_you_located)
sal_clean_filt |> 
  mutate(country = case_when(
    !is.na(country) ~ country,
    where_are_you_located %in% USA ~ 'USA',
    !is.na(what_country_do_you_work_in) ~ what_country_do_you_work_in,
    TRUE ~ NA
  )
  ) |> 
  
  count(country, what_country_do_you_work_in,  where_are_you_located) |> 
  ```
