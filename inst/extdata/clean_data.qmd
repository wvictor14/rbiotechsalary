...
sheet_names <- gs4_get(google_sheets_url )$sheets$name
...
contains all degrees (not just highest)

contains a bunch of NAs

These NAs are due to question change in 2022 and 2023:

```{r}
sal_clean_filt |>  
  count(
    sheet_year, what_degrees_do_you_have, highest_achieved_formal_education) |>
  gt::gt()
```

but let's simplify to the highest degree:
...
sal_clean_filt <- sal_clean_filt |> 
  mutate(
    experience_highest_degree =  case_when(
      !is.na(highest_achieved_formal_education) ~ 
        highest_achieved_formal_education |> 
        str_remove('or Equivalent') |> 
        str_trim() |> 
        str_replace('M\\.D\\.\\/PharmD\\/D\\.D\\.S\\.', 'MD'),
      
      # 2023 and 2024
      !is.na(what_degrees_do_you_have) ~ what_degrees_do_you_have |> 
        str_extract('(?<=,|^)[^,]+$') |>  # get the last title in series
        str_replace('M\\.D\\.', 'MD') |> 
        str_remove('or Equivalent') |> 
...
sal_clean_filt |>  
  count(
    is.na(experience_highest_degree), 
...
sal_clean_filt |> count(highest_achieved_formal_education) |>  gt::gt()

# NAs in cleaned column should be where respondents left blank
sal_clean_filt |> 
  filter(is.na(experience_highest_degree)) |> 
  count(experience_highest_degree, what_degrees_do_you_have, 
        highest_achieved_formal_education) |>  
  gt()
```
